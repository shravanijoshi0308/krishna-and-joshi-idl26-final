...
print("\n" + "="*50)
print("EXPERIMENT 3: Transfer learning (ResNet18) from orgs-trained model")
print("="*50)

resnet_transfer = models.ResNet18(in_channels=1, num_classes=11).to(device)
resnet_transfer.load_state_dict(torch.load("ResNet18_orgs.pth", map_location=device))
for param in resnet_transfer.parameters():
    param.requires_grad = False
for param in resnet_transfer.classifier.parameters():
    param.requires_grad = True

criterion_resnet = nn.CrossEntropyLoss()
optimizer_resnet = optim.Adam(filter(lambda p: p.requires_grad, resnet_transfer.parameters()), lr=0.0001)
trainer_resnet = Trainer(resnet_transfer, criterion_resnet, optimizer_resnet, device)
trainer_resnet.fit(train_loader, val_loader, epochs=10)

test_loss_r, test_acc_r, test_precision_r, test_recall_r, test_f1_r = trainer_resnet.evaluate(test_loader)
print(f"ResNet18 Transfer Test Results | Loss: {test_loss_r:.4f} | Acc: {test_acc_r:.2f}% | Precision: {test_precision_r:.4f} | Recall: {test_recall_r:.4f} | F1: {test_f1_r:.4f}")

# Add ResNet18's result to the SAME results list (don't recreate it)
results.append({
    "experiment": "Transfer (ResNet18, frozen backbone)",
    "test_loss": test_loss_r,
    "test_acc": test_acc_r,
    "precision": test_precision_r,
    "recall": test_recall_r,
    "f1": test_f1_r
})

print("\n" + "="*50)
print("FULL SCARCE-DATA BENCHMARK MATRIX (organs dataset) for ResNet18")
print("="*50)
for r in results:
    print(f"{r['experiment']:<38} | Loss: {r['test_loss']:.4f} | Test Acc: {r['test_acc']:6.2f}% | "
          f"Precision: {r['precision']:.4f} | Recall: {r['recall']:.4f} | F1: {r['f1']:.4f}")